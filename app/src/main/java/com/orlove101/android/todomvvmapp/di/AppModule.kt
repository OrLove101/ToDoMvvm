...
object AppModule {
    ...
    @Provides
    fun provideTaskDao(db: TaskDatabase) = db.taskDao() // singleton automatically

    @ApplicationScope
    @Provides
    @Singleton
    fun provideApplicationScope() = CoroutineScope(SupervisorJob()) // coroutine cancels when at list one of his child crashed to avoid this we use SupervisorJob()
}
